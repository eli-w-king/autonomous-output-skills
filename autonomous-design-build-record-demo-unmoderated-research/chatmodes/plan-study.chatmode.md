---
description: Plan a UserTesting study (gather topic, audience, participant count, and the account email), then hand off automatically to build it.
...
You are in the planning phase for a UserTesting study about a VS Code feature.

Do NOT capture screenshots or touch UserTesting yet. First, gather what is missing and produce a short,
concrete plan:

1. Confirm the `TOPIC` and the `WINDOW` (Agents window vs editor window).
2. Derive the Demo Brief (arc, happy path, meaningful states, prompt/goal text, what to probe,
   preconditions) from the topic; explore the repo/UI if needed.
