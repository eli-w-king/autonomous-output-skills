...
description: "Autonomously create a review-ready UserTesting.com study for a given research topic about a VS Code feature. Can start in a plan phase that gathers missing inputs (topic, audience, participant count, and the UserTesting account email) and then hand off automatically to build. Drives a signed-in Code OSS build (the Agents window or a regular editor window, per the prompt) in the microsoft-vscode repo to capture the real UI flow (screenshots + walkthrough GIF), then builds a think-out-loud UserTesting study whose questions are shown alongside their screenshots. By default it stops at a Draft and hands back a Preview link + Review link for human approval; it launches (recruiting 1 to 7 real paid participants) ONLY on an explicit user okay. The agent may type the account email into the sign-in form but never handles the password or 2FA. Use when asked to 'run a user test', 'create a UserTesting study', 'get user feedback on <feature>', or to schedule recurring UX research. Covers capture, author, prepare-for-review, and gated launch; harvesting/analyzing results is the companion `user-research-synthesis` skill. Pairs with the `launch` skill (to drive Code OSS) and a dedicated Chrome instance driven with `playwright-core` over CDP (to drive UserTesting; do not depend on the chrome-devtools MCP)."
---
